...
class ProteinBertModelWithStructuralAdatperTime(nn.Module):
    @classmethod
    ...
    def __init__(self, args, alphabet):
        ...
        self.embed_structures = nn.Sequential(
            ChebyKAN(
                layers_hidden=[self.args.encoder.d_model, self.args.embed_dim],
                degree=5,
                scale_base=1.0,
                scale_cheby=1.0,
            ...
            # nn.Linear(self.args.embed_dim, self.args.embed_dim, bias=True),
            ChebyKAN(
                layers_hidden=[self.args.embed_dim, self.args.embed_dim],
                degree=5,
                scale_base=1.0,
                scale_cheby=1.0,
                base_activation=torch.nn.SiLU,
    ...
    def initialize_weights(self):
        ...
        self.apply(_basic_init)
            ...
            # 获取并初始化KAN第一层
        kan1 = self.embed_structures[0]
        if hasattr(kan1, 'linear1'):
            nn.init.normal_(kan1.linear1.weight, std=0.02)
        if hasattr(kan1, 'linear2'):
            nn.init.normal_(kan1.linear2.weight, std=0.02)
        
        # 获取并初始化KAN第二层
        kan2 = self.embed_structures[1]
        if hasattr(kan2, 'linear1'):
            nn.init.normal_(kan2.linear1.weight, std=0.02)
        if hasattr(kan2, 'linear2'):
            nn.init.normal_(kan2.linear2.weight, std=0.02)
        ...
        kan3 = self.embed_timesteps.mlp[0]
        if hasattr(kan3, 'linear1'):
            nn.init.normal_(kan3.linear1.weight, std=0.02)
        if hasattr(kan3, 'linear2'):
            nn.init.normal_(kan3.linear2.weight, std=0.02)
        
        # 获取并初始化KAN第二层
        kan4 = self.embed_timesteps.mlp[1]
        if hasattr(kan4, 'linear1'):
            nn.init.normal_(kan4.linear1.weight, std=0.02)
        if hasattr(kan4, 'linear2'):
            nn.init.normal_(kan4.linear2.weight, std=0.02)
    ...
    def forward(self, tokens, alpha_t_bar, context, timesteps, encoder_out, repr_layers=[], need_head_weights=False, return_contacts=False):
        ...
        if context is not None:
            x_context = self.embed_scale * self.embed_tokens(context)
            # print(x_context.shape)
            # print(x.shape)
            alpha_t_bar = alpha_t_bar.to(x.device)
            x_context = x_context.to(x.device)
            x = (1 - alpha_t_bar.unsqueeze(-1)) * x + alpha_t_bar.unsqueeze(-1) * x_context
        ...
        x = x + self.embed_positions(tokens) # B, T, H
        ...
        # print(encoder_out['aligned_feats'].shape)
        # print(x.shape)
...
class TransforerLayerWithStructralAdapterTime(nn.Module):
    ...
    def _init_submodules(self, add_bias_kv, use_esm1b_layer_norm):
        ...
        # self.fc1 = ChebyKAN(
        #         layers_hidden=[self.embed_dim, self.ffn_embed_dim],
        #         degree=5,
        #         scale_base=1.0,
        #         scale_cheby=1.0,
        #         base_activation=torch.nn.SiLU,
        ...
        # self.fc2 = ChebyKAN(
        #         layers_hidden=[self.ffn_embed_dim, self.embed_dim],
        #         degree=5,
        #         scale_base=1.0,
        #         scale_cheby=1.0,
        #         base_activation=torch.nn.SiLU,
    ...
    def forward(
        ...
        x = x + self.forward_adapter(x, c, encoder_out, attn_mask=self_attn_mask, attn_padding_mask=self_attn_padding_mask)
        return x, attn

    def forward_adapter(self, x, c, encoder_out, attn_mask, attn_padding_mask):
...
class MyNormalizedResidualBlock(NormalizedResidualBlock):
    def __init__(self, layer: nn.Module, embedding_dim: int, dropout: float = 0.1):
        super().__init__(layer, embedding_dim, dropout)
        self.layer_norm = ESM1bLayerNorm(self.embedding_dim, elementwise_affine=False)
