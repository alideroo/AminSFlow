...
from src.modules import diffusion_utils
...
class PredefinedNoiseScheduleDiscrete(torch.nn.Module):
    ...
    def __init__(self, noise_schedule, timesteps):
        ...
        if noise_schedule == 'cosine':
            ...
            betas = diffusion_utils.cosineSD_beta_schedule_discrete(timesteps, d=4)
        elif noise_schedule == 'interpolation':
            betas = diffusion_utils.interpolation_beta_schedule_discrete(timesteps)
...
class InterpolationTransition:
    ...
    def get_Qt(self, beta_t, X_T, node_mask, device):
        """
        X_T (bs, n, dx)

        Returns transition matrix for X
        """
    ...
    def get_Qt_bar(self, alpha_bar_t, X_T, node_mask, device):
        """
        alpha_bar_t: (bs, 1)
        X_T: (bs, n, dx)

        Returns transition matrices for X
        """
